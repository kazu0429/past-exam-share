    ...
    return(
        ...
        <main className="flex-1 bg-indigo-50">
            ...
            <form className="w-full" onSubmit={UploadExam}>
                <input type="text" placeholder="記事タイトル" className="m-5 w-5/6  rounded-xl border  border-gray-300 p-2 text-xl" onChange={(e) => setTitle(e.target.value)} required/>
                <div className="flex">
                    <input placeholder="科目名" className="m-5 w-2/3 rounded-xl border border-gray-300 p-2 text-xl" onChange={(e) => setSubjectName(e.target.value)} required/>
                    <select placeholder="学部" name="faculty" className="m-5 rounded-xl border border-gray-300 p-2" required onChange={(e) => setFaclty(e.target.value)}>
                            {faculty_list.map((option, i) => (
                                <option value={option} key={i}>{option}</option>
                            ))}
                    </select>
                </div>
                <textarea rows={5} placeholder="試験の傾向など" className="m-5 w-5/6 rounded-xl border border-gray-300 p-2"  onChange={(e) => setDiscription(e.target.value)}/>
                <div className="ml-5 flex flex-wrap items-center gap-x-3">
                ...
                <div className="m-5">
                    {fielType === ".pdf" ? 
                        <p>{pdf?.name}</p>:images.map((image, i) => (
                            <p key={i} className="ml-5 flex items-center gap-x-3 ">{image.name}
